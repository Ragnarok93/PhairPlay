...
**Tech Stack:** Kotlin, Android SDK 25–35, WifiP2pManager, RTSP/RTP/MPEG-TS, MediaCodec, AudioTrack, Google Cast Connect, AndroidX Media3 1.9.4, JUnit/MockK/Robolectric, GitHub Actions.
...
- [x] Add tests for API 25–32 location permission behavior and API 33+ NEARBY_WIFI_DEVICES behavior.
- [x] Guard every newer Wi-Fi API call by SDK level.
- [x] Limit legacy location permissions to Android 12L and below in the manifest while preserving Fire OS 6 compatibility.
- [x] Keep target/compile SDK unchanged and verify both flavors compile.
...
- [x] Parse RTP v2 headers, sequence number, timestamp, marker, CSRC/extension/padding.
- [x] Validate MPEG-TS 188-byte framing and continuity.
- [x] Parse PAT/PMT and PES boundaries and surface elementary-stream packets with PTS.
- [x] Drop malformed/lost data without crashing and wait for the next recoverable boundary.
...
- [x] Ensure AirPlay/Miracast/Cast cannot simultaneously own the Surface/audio path.
- [x] Reuse MediaCodec H.264 decode where compatible and add PTS-aware render timing.
- [x] Add mandatory LPCM playback.
- [ ] Add codec-gated optional AAC decode only after runtime capability advertisement is implemented.
- [x] Propagate Miracast CONNECTED/streaming/error state to UI and notifications.
...
- [x] Add Google-TV-only Media3 ExoPlayer/HLS/DASH dependencies at stable 1.9.4, the newest line compatible with the retained compileSdk 35 toolchain.
- [x] Register MediaLoadCommandCallback before forwarding load intents.
- [x] Route launch/load intents from Activity creation and `onNewIntent`.
- [x] Implement load/play/pause/seek/stop/status/error propagation.
- [x] Reject unsupported or missing content URLs with Cast media errors.
- [x] Keep Fire TV flavor free of Cast/Media3 receiver classes.
...
- [ ] Verify no Google Play Services / Media3 dependencies enter the Fire TV runtime classpath (CI guard added; final head verification pending).
